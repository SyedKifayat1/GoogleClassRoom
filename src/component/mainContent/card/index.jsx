...
const Card = ({ bgColor, initial, classTitle, batch, teacher, onSelectHeading }) => (
    <div className="max-w-xs w-full rounded-lg border hover:shadow-xl bg-white overflow-hidden">
      <div className={`bg-cover bg-center h-24 flex flex-col justify-end p-4 text-white relative ${bgColor}`}>
        ...
        <Link to={`/subject-details/${classTitle}`}>
          ...
          <h2 className="text-lg font-semibold cursor-pointer" onClick={() => onSelectHeading(classTitle)}>{classTitle}</h2>
        </Link>
      ...
      <div className="p-6 min-h-[8rem]">
        <div className="content pb-1">
          ...
          <Link to={`/assignment-details/${classTitle}`} onClick={() => onSelectHeading(classTitle)}><h3 className="text-sm font-bold mt-1">Assignment</h3></Link>
        </div>
